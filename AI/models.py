...
class Notification(models.Model):
    id = models.AutoField(primary_key=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE, null=True, blank=True)
    user_contact = models.CharField(max_length=255, null=True, blank=True)
...
class RouteMap(models.Model):
    id = models.AutoField(primary_key=True)
    lost_product = models.ForeignKey(LostProduct, on_delete=models.CASCADE, null=True, blank=True, related_name='route_maps')
    ...
    def __str__(self):
        ...
        elif self.lost_product:
            return f"RouteMap {self.id} for {self.lost_product.name}"
